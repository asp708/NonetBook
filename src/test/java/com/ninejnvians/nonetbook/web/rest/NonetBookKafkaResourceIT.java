...
import com.ninejnvians.nonetbook.config.KafkaProperties;
...
class NonetBookKafkaResourceIT {
    ...
    void setup() {
        ...
        NonetBookKafkaResource kafkaResource = new NonetBookKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/nonet-book-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/nonet-book-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
